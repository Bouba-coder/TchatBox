...
import {PENDING_REQUESTS_KEY, OUT_GOING_REQUESTS_KEY, ALL_FRIENDS_KEY} from '../../../../constants/queryKeys'
import {cancelPendingRequestApi, acceptPendingRequestApi} from '../../../../api/friend'
...
function pendingStatus(user, request) {
    if (isIncoming(user, request)) 
        return PENDING_WAY.INCOMING
                            ...
                            isIncoming(user, pending)
                                ? pending
                                    ?.from
                                        ...
                                        : pending
                                            ?.to
                                                ?.color} text-white hover:text-tchatbox-100 rounded-full`}>
                            <TchatBox className='w-5 h-5'/>
                        </div>
                        <span
                            className='bg-tchatbox-green w-3 h-3 rounded-full absolute right-0 bottom-0 -mr-1 mb-1'></span>
                    </div>
                ...
                <div className='flex'>
                    {
                        isIncoming(user, pending) && (
                            <button
                                ...
                                <MarkIcon
                                    className='fill-current w-5 h-5 text-tchatbox-topIcons hover:text-tchatbox-lightGreen'/>
                            </button>
                        )
                    }
                    ...
                    <button
                        ...
                        className='flex items-center justify-center p-2 mx-1 rounded-full bg-tchatbox-bgSecondary focus:outline-none'>
                        <CloseIcon
                            className='fill-current w-5 h-5 text-tchatbox-topIcons hover:text-tchatbox-redNotif'/>
                    </button>
                </div>
            </div>
        </li>
